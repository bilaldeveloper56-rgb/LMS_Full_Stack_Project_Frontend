...
import { updateNoticeSchema, NOTICE_PRIORITIES, TARGET_AUDIENCES } from '../schemas/notice.schema';
...
export function EditNoticePage() {
  ...
  const handleFormSubmit = async (data) => {
    ...
    await updateNoticeMutation.mutateAsync({ id, payload });
    navigate(`/notices/${id}`);
  };
    ...
    <div className="space-y-6 max-w-3xl">
      ...
      <div className="flex items-center justify-between border-b border-border pb-4">
        <div>
          ...
          <p className="mt-1 text-sm text-text-secondary">
            Update notice content, priority level, or target audience
          </p>
        </div>
        <Link to={`/notices/${id}`}>
      ...
      <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-6">
        <Card className="p-6 space-y-5 border border-border bg-surface shadow-2xs">
          ...
          <Input
            label="Notice Title *"
            placeholder="e.g. Updated Schedule for Sports Day"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
            <label className="flex items-center gap-2.5 p-3 rounded-lg border border-border cursor-pointer hover:bg-surface-muted/40 transition-colors">
              ...
              <div>
                ...
                <span className="text-[11px] text-text-muted">Keep this announcement highlighted at top</span>
              </div>
            </label>
